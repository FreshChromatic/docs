...
新版文檔需要讓第一次接觸 ChunkRevive 的伺服器管理員，能在不閱讀完整指令表或設定表的情況下，安全完成第一次區塊重生；也要讓有經驗的管理員能快速查到指令、設定、相容性和故障處理資訊。
...
4. 指令、設定、權限與相容性資料完整，且能追溯到程式碼中的真實行為。
...
- 已涵蓋安裝、常見工作流、指令、部分設定、相容性、整合與開發者資訊。
...
| 教學與參考混在一起 | `usage/workflows` 同時解釋觀念、列步驟、補指令語法 | 初學者資訊過載，熟練者又難以快速查找 |
| 高風險操作曝光不足 | `reset`、`delete`、`prune` 與一般重生共同出現在指令集合中 | 使用者可能把不同資料破壞程度的操作視為同一類 |
| 設定頁不完整 | 目前只展示部分 `PluginConfig` 欄位 | 無法作為可信賴的設定參考 |
| 功能覆蓋不足 | 刪除工作恢復、結構偵測/保護/刷新、調校模式、世界存取策略等缺少獨立說明 | 重要功能只能從指令名稱推測 |
| 缺少故障處理入口 | 啟動失敗、版本不符、工作卡住、記憶體壓力、Residence 排除等未形成排查流程 | 問題發生時只能搜尋零散頁面 |
| 缺少頁面責任邊界 | 相同安全提醒和操作說明散落在多頁 | 後續容易出現重複、矛盾與過期內容 |
| 缺少維護規則 | 指令與設定未和來源檔案建立核對表 | 程式更新後文檔容易落後 |
...
| Diátaxis | 分離學習、任務、查詢與理解的內容 | 避免在指令表內插入長篇教學，也避免新手教學塞入全部設定欄位 |
...
| **Delete** | 等待區塊保持 cold，再移除指定 chunk 的 terrain、entity 和 POI 儲存項目；新地形在該區塊下次由伺服器載入時按需生成 | 零散區塊、只選中部分 Region、可接受稍後才重生，或希望降低當下 CPU/Heap 負擔的資源世界維護 |
...
這不是因為 Regen 不可使用，而是因為 Delete/Prune 的工作邊界更短：外掛負責安全移除舊資料，之後把重生交回伺服器處理。相較之下，Regen 必須由版本專用 NMS 立即重演世界生成、序列化、寫盤和 live chunk 更新。
...
指令參考保持精簡、完整和中立。長流程、背景原理與效能建議放到對應指南。

權限不再建立獨立的 `permissions.mdx`。`commands.mdx` 頁首先提供權限總表，再在每個指令項目中標示實際所需權限，讓讀者不用在指令頁與權限頁之間往返。權限總表至少包含：
...
- 程式碼區塊：設定範例使用 `yaml`，指令使用 `text` 或 `bash`，並加上能理解目的的標題。
- 表格：適合指令、設定與比較；教學步驟不要全部塞進表格。

### 7.3 語言與術語

- 使用繁體中文（臺灣），保留產品名、指令、設定鍵與技術標識的英文。
- 統一使用「區塊」表示 chunk、「重生」或「重生」需選定其中一種作為主詞；正式撰寫前建立術語表。
- `region` 第一次出現寫成「Anvil Region（32×32 區塊）」；後續維持同一稱呼。
- 區分方塊座標、區塊座標與 Region 座標，每個含座標的頁面都要標明單位。
- 不使用「顯然、簡單、只要」等低估風險或使用者經驗的語氣。
- 危險指令使用直接句型，明確寫出會變更或刪除哪些資料。
...
| 指令語法、參數、執行者限制 | `ChunkReviveCommand.java` | 建立指令盤點表，與註冊節點逐項核對 |
...
- 匯出完整指令樹、參數、權限、玩家/主控台限制與確認門檻。
- 匯出所有設定鍵、預設值和註解。
- 確認每個 NMS adapter 對應的公開版本名稱。
- 盤點資料庫表、持久化檔案與重啟後恢復行為。
- 建立術語表與操作影響說明。

**交付物**：指令矩陣、設定矩陣、功能覆蓋矩陣、術語表。
...
**驗收情境**：未接觸過 ChunkRevive 的管理員能在測試世界完成一次重生，且知道哪些指令尚不應使用。
...
- 完整指令參考。
- 完整設定鍵參考。
- 在指令參考中完成權限總表，並完成相容性、檔案與儲存參考。
- Residence、PacketEvents 及開發者整合邊界。

**驗收情境**：所有已註冊指令和 `PluginConfig` 公開設定都有文檔位置，覆蓋率 100%。
